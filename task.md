...
- [x] `employee-detail.ts`: các ngày mặc định (thêm chức vụ/lương/dự án, bộ lọc chấm công) chỉ tính 1 lần lúc tải trang — cần tính lại mỗi lần mở modal/áp dụng filter. Đã sửa: `openPositionModal()`/`openIncomeModal()`/`openProjectsModal()` giờ tự tính lại "hôm nay" mỗi lần mở thay vì dùng giá trị cũ từ lúc tải trang. Nhân tiện phát hiện thêm bug thật: `initializeSubFormData()` dùng `new Date().toISOString()` (UTC) cho ngày chức vụ/lương/dự án nhưng lại dùng công thức local-safe khác cho bộ lọc chấm công — 2 công thức lệch nhau tới ±1 ngày tùy múi giờ. Đã gộp về 1 hàm util dùng chung `core/utils/date.util.ts` (`todayLocalDateString`/`startOfMonthLocalDateString`/`toLocalDateString`), đồng thời dọn luôn 3 chỗ khác đang tự viết lại y hệt logic này (`dashboard.ts`, `staff-profile.ts`, `date-picker.ts`) — đúng nguyên tắc "không lặp code" trong AGENTS.md. Verify: lint + tsc + build + 75 test Vitest đều pass; kiểm tra trực tiếp trên browser cả 3 modal đều hiện đúng ngày hiện tại (19/07/2026) khi mở.
- [x] `employee-detail.ts`: tách bớt full-page spinner khi reload sau khi lưu (hiện y hệt lần tải đầu, giật cục không cần thiết). Đã sửa: `loadEmployeeDetails(silent = false)` — khi gọi lại sau khi lưu/xóa (7 chỗ: base profile, position, income, skills, projects, delete position, delete income) truyền `silent = true` để bỏ qua `isLoading`/skeleton toàn trang; lỗi refresh trong chế độ silent cũng bị bỏ qua (không đè `errorMsg` lên nội dung đang hiển thị) vì thao tác lưu/xóa đã tự báo kết quả riêng cho người dùng rồi — theo đúng chính sách "im lặng bỏ qua có chủ đích" đã áp dụng ở `RealtimeService`. `ngOnInit` (tải lần đầu) và nút "Thử lại" vẫn giữ hành vi cũ (silent=false mặc định). Verify: lint + tsc + 75 test Vitest pass; test trực tiếp trên browser — bổ nhiệm chức vụ mới, trang cập nhật tại chỗ không hề chớp/giật về skeleton.
